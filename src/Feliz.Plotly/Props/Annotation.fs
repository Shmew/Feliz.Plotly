...
type annotation =
    ...
    /// Sets the annotation's x position.
    static member inline x (values: seq<bool>) = Interop.mkAnnotationAttr "x" (values |> ResizeArray)
    /// Sets the annotation's x position.
    ...
    /// Sets the annotation's x position.
    static member inline x (values: seq<int>) = Interop.mkAnnotationAttr "x" (values |> ResizeArray)
    /// Sets the annotation's x position.
    static member inline x (value: float) = Interop.mkAnnotationAttr "x" value
    /// Sets the annotation's x position.
    static member inline x (values: seq<float>) = Interop.mkAnnotationAttr "x" (values |> ResizeArray)
    /// Sets the annotation's x position.
    static member inline x (value: string) = Interop.mkAnnotationAttr "x" value
    /// Sets the annotation's x position.
    static member inline x (values: seq<string>) = Interop.mkAnnotationAttr "x" (values |> ResizeArray)
    ...
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (values: seq<bool>) = Interop.mkAnnotationAttr "xclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    ...
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (values: seq<int>) = Interop.mkAnnotationAttr "xclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (value: float) = Interop.mkAnnotationAttr "xclick" value
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (values: seq<float>) = Interop.mkAnnotationAttr "xclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (value: string) = Interop.mkAnnotationAttr "xclick" value
    /// Toggle this annotation when clicking a data point whose `x` value is `xclick` rather than the annotation's `x` value.
    static member inline xclick (values: seq<string>) = Interop.mkAnnotationAttr "xclick" (values |> ResizeArray)
    ...
    /// Sets the annotation's y position.
    static member inline y (values: seq<bool>) = Interop.mkAnnotationAttr "y" (values |> ResizeArray)
    /// Sets the annotation's y position.
    ...
    /// Sets the annotation's y position.
    static member inline y (values: seq<int>) = Interop.mkAnnotationAttr "y" (values |> ResizeArray)
    /// Sets the annotation's y position.
    static member inline y (value: float) = Interop.mkAnnotationAttr "y" value
    /// Sets the annotation's y position.
    static member inline y (values: seq<float>) = Interop.mkAnnotationAttr "y" (values |> ResizeArray)
    /// Sets the annotation's y position.
    static member inline y (value: string) = Interop.mkAnnotationAttr "y" value
    /// Sets the annotation's y position.
    static member inline y (values: seq<string>) = Interop.mkAnnotationAttr "y" (values |> ResizeArray)
    ...
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (values: seq<bool>) = Interop.mkAnnotationAttr "yclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    ...
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (values: seq<int>) = Interop.mkAnnotationAttr "yclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (value: float) = Interop.mkAnnotationAttr "yclick" value
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (values: seq<float>) = Interop.mkAnnotationAttr "yclick" (values |> ResizeArray)
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (value: string) = Interop.mkAnnotationAttr "yclick" value
    /// Toggle this annotation when clicking a data point whose `y` value is `yclick` rather than the annotation's `y` value.
    static member inline yclick (values: seq<string>) = Interop.mkAnnotationAttr "yclick" (values |> ResizeArray)
    ...
    /// Sets the annotation's z position.
    static member inline z (values: seq<bool>) = Interop.mkAnnotationAttr "z" (values |> ResizeArray)
    /// Sets the annotation's z position.
    ...
    /// Sets the annotation's z position.
    static member inline z (values: seq<int>) = Interop.mkAnnotationAttr "z" (values |> ResizeArray)
    /// Sets the annotation's z position.
    static member inline z (value: float) = Interop.mkAnnotationAttr "z" value
    /// Sets the annotation's z position.
    static member inline z (values: seq<float>) = Interop.mkAnnotationAttr "z" (values |> ResizeArray)
    /// Sets the annotation's z position.
...
module annotation =
    ...
    type axref =
        ...
        static member inline x (anchorId: int) = Interop.mkAnnotationAttr "axref" (sprintf "x%s" (if anchorId > 1 then (anchorId |> string) else ""))
    ...
    type ayref =
        ...
        static member inline y (anchorId: int) = Interop.mkAnnotationAttr "ayref" (sprintf "y%s" (if anchorId > 1 then (anchorId |> string) else ""))
    ...
    type xref =
        ...
        static member inline x (anchorId: int) = Interop.mkAnnotationAttr "xref" (sprintf "x%s" (if anchorId > 1 then (anchorId |> string) else ""))
    ...
    type yref =
        ...
        static member inline y (anchorId: int) = Interop.mkAnnotationAttr "yref" (sprintf "y%s" (if anchorId > 1 then (anchorId |> string) else ""))
