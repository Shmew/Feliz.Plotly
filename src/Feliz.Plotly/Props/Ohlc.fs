...
type ohlc =
    ...
    /// Assigns id labels to each datum. These ids for object constancy of data points during animation. Should be an array of strings, not numbers or any other type.
    static member inline ids (values: seq<int []>) = Interop.mkOhlcAttr "ids" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Assigns id labels to each datum. These ids for object constancy of data points during animation. Should be an array of strings, not numbers or any other type.
    ...
    /// Assigns extra data each datum. This may be useful when listening to hover, click and selection events. Note that, *scatter* traces also appends customdata items in the markers DOM elements
    static member inline customdata (values: seq<int []>) = Interop.mkOhlcAttr "customdata" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Assigns extra data each datum. This may be useful when listening to hover, click and selection events. Note that, *scatter* traces also appends customdata items in the markers DOM elements
    ...
    /// Sets the x coordinates. If absent, linear coordinate will be generated.
    static member inline x (values: seq<int []>) = Interop.mkOhlcAttr "x" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Sets the x coordinates. If absent, linear coordinate will be generated.
    ...
    /// Sets the open values.
    static member inline open' (values: seq<int []>) = Interop.mkOhlcAttr "open" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Sets the open values.
    ...
    /// Sets the high values.
    static member inline high (values: seq<int []>) = Interop.mkOhlcAttr "high" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Sets the high values.
    ...
    /// Sets the low values.
    static member inline low (values: seq<int []>) = Interop.mkOhlcAttr "low" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Sets the low values.
    ...
    /// Sets the close values.
    static member inline close (values: seq<int []>) = Interop.mkOhlcAttr "close" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// Sets the close values.
