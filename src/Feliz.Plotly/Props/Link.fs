...
type link =
    ...
    /// The shown name of the link.
    static member inline label (values: seq<int option []>) = Interop.mkLinkAttr "label" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// The shown name of the link.
    ...
    /// Sets the `link` color. It can be a single value, or an array for specifying color for each `link`. If `link.color` is omitted, then by default, a translucent grey link will be used.
    static member inline color (values: seq<int>) = Interop.mkLinkAttr "color" (values |> Array.ofSeq)
    /// Sets the `link` color. It can be a single value, or an array for specifying color for each `link`. If `link.color` is omitted, then by default, a translucent grey link will be used.
    static member inline color (values: seq<float>) = Interop.mkLinkAttr "color" (values |> Array.ofSeq)
    ...
    /// An integer number `[0..nodes.length - 1]` that represents the source node.
    static member inline source (values: seq<int option []>) = Interop.mkLinkAttr "source" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// An integer number `[0..nodes.length - 1]` that represents the source node.
    ...
    /// An integer number `[0..nodes.length - 1]` that represents the target node.
    static member inline target (values: seq<int option []>) = Interop.mkLinkAttr "target" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// An integer number `[0..nodes.length - 1]` that represents the target node.
    ...
    /// A numeric value representing the flow volume value.
    static member inline value (values: seq<int option []>) = Interop.mkLinkAttr "value" (values |> Seq.map ResizeArray |> Array.ofSeq)
    /// A numeric value representing the flow volume value.
